...
public class RigisterZuKeThreeActivity extends ActionBarActivity implements OnClickListener{
	...
	private EditText login_phone,login_yzm,login_password,login_password2,login_mobile,login_name;
							...
							if(list != null && list.get("grade").equals(ConstantValue.YEZHU)) {
								if(list.get("xqid").equals(xiaoId)) {
									if(list.get("fanghao").equals(fangId)) {
										...
										PromptManager.showToast(getApplicationContext(), "您选择的小区房号和输入的房主号码不一致");
									}
								} else {
									PromptManager.showToast(getApplicationContext(), "您选择的小区房号和输入的房主号码不一致");
								}
				...
				case RIGISTER:
					...
					if(r != null) {
						if(r.getRows().equals("1")) {
							...
							Intent intent = new Intent(RigisterZuKeThreeActivity.this,LoginActivity.class);
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		getActionBar().hide();
		setContentView(R.layout.activity_rigister_zuke);
	...
	public void onClick(View v) {
		...
		case R.id.login_rig_visitor:
			phone = login_mobile.getText().toString().trim();
			...
			if(StringUtils.isBlank(phone2)) {
				PromptManager.showToast(getApplicationContext(), "请输入房主手机号");
				return;
			}
			if(!StringUtil.isPhone(phone2)) {
				PromptManager.showToast(getApplicationContext(), "请输入正确格式的房主手机号");
				return;
			}
			checkPhone(phone2);
			break;
		case R.id.login_but:
			...
			String phone3 = login_phone.getText().toString().trim();
			if(StringUtils.isBlank(phone3)) {
				PromptManager.showToast(getApplicationContext(), "请输入房主手机号");
				return;
			}
			if(!StringUtil.isPhone(phone3)) {
				PromptManager.showToast(getApplicationContext(), "请输入正确格式的房主手机号");
				return;
			}
			...
			rigister(name,mobile,phone3,password1);
			break;
		}
	}
	 ...
	 * @param password1
	 */
	private void rigister(final String name,final String mobile,final String phone2, final String password1) {
		new Thread(){
			@Override
			public void run() {
				super.run();
				r = WebServiceUtils.add2(mobile,city,type,xiaoId,password1,name,phone2,"",fangId);
