...
public class MemoryManager {
	...
	private static final int MAXMEMORY=300*1024*1024;//程序运行的最大内�?模拟�?0-16m)
	/**
	 * 判断系统是否在低内存下运�?
	 * @param context
	 * @return
	...
	public static long getAvailableInternalMemorySize() {
		...
		StatFs stat = new StatFs(path.getPath());// �?��模拟linux的df命令的一个类,获得SD卡和手机内存的使用情�?
	...
	public static long getTotalExternalMemorySize() {
		if (externalMemoryAvailable()) {
			File path = Environment.getExternalStorageDirectory();// 获取外部存储目录�?SDCard
			StatFs stat = new StatFs(path.getPath());
			long blockSize = stat.getBlockSize();
